...
def train(cfg: PEFTTrainPipelineConfig):
    ...
    for peft_module in peft_modules:
        for discriminator_id in range(peft_module.num_discriminators):
            ...
            train_discriminator_metrics[f"running_mean_{key}"] = AverageMeter(f"loss_{key}", ":.3f")
            train_discriminator_metrics[f"running_std_{key}"] = AverageMeter(f"loss_{key}", ":.3f")
            train_discriminator_metrics[f"num_batches_tracked_{key}"] = AverageMeter(f"loss_{key}", ":.3f")
