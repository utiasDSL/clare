...
class WandBLogger:
    ...
    def log_dict(
        ...
        if mode not in {"train", "eval", "train_discriminator", "continual_learning", "eval_after_prune"}:
    ...
    def log_figure(self, figure_path, step: int, mode: str = "train"):
        if mode not in {"train", "eval", "train_discriminator", "continual_learning", "eval_after_prune"}:
            raise ValueError(mode)

        wandb_figure = self._wandb.Image(figure_path)
        self._wandb.log({f"{mode}/figure": wandb_figure}, step=step)
